#!/usr/bin/env bash
#
# Verify Ollama's /api/multivectors against a direct llama.cpp run of the
# same model: Ollama serves the per-token rows from llama-server's
# /embeddings endpoint (pooling none) verbatim, so the matrices must match a
# standalone llama-server within float-serialization tolerance.
#
# Usage:
...
echo "==> requesting token embeddings from both servers"
jq -n --arg p "$PROMPT" '{content:$p, embd_normalize:-1}' \
...
max_diff = 0.0
for a, b in zip(rows, orows):
    if len(a) != len(b):
        print(f"FAIL: dim mismatch ({len(a)} vs {len(b)})", file=sys.stderr)
        sys.exit(1)
    for x, y in zip(a, b):
        max_diff = max(max_diff, abs(x - y))

print(f"rows={len(rows)} dim={len(rows[0]) if rows else 0} max_abs_diff={max_diff:.6g}")
if max_diff > tol:
    print(f"FAIL: max abs diff {max_diff:.6g} exceeds tolerance {tol:.6g}", file=sys.stderr)
    sys.exit(1)

print("PASS: Ollama serves llama.cpp's token rows verbatim")
PY
